...
class GameScene(Scene):
    ...
    def update(self):
        ...
        char_tile_x = int((self.character.x + self.character.frame_width // 2) // self.tile_map.tile_size)
        char_tile_y = int((self.character.y + self.character.frame_height // 2) // self.tile_map.tile_size)
        
        # Проверяем коллизию с тайлами-препятствиями
        if self.tile_map.is_collision(char_tile_x, char_tile_y):
            # Откатываем позицию
