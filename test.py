...
def main():
    ...
    pygame.display.set_caption("Тайловая анимация - Автоматическое определение размеров")
    clock = pygame.time.Clock()
    
    # Создаем демо-спрайты или используем свои
    #walk_path, idle_path = create_demo_sprites()
    walk_path, idle_path = "pics/main_person/pers.png", "pics/main_person/idle.png"

    # Создаем персонажа
    character = Char(
        x=SCREEN_WIDTH // 2,
        y=SCREEN_HEIGHT // 2,
        walk_sprite_path=walk_path,
        idle_sprite_path=idle_path
    )
    ...
    while running:
        ...
        if show_info:
            ...
            direction_text = {
                ...
                "down": "WALK DOWN (Вниз ↓)",
                "up": "WALK UP (Вверх ↑)",
                "left": "WALK LEFT (Влево ←)",
                "right": "WALK RIGHT (Вправо →)"
            }
            
            current_action = direction_text.get(character.current_animation, "UNKNOWN")
            frames_count = len(character.animations.get(character.current_animation, []))
            
            # Фон для текста
            text_bg = pygame.Surface((350, 280))
            ...
            info_lines = [
                ...
                f"Кадр: {character.current_frame + 1}/{frames_count}",
                f"Позиция: ({int(character.x)}, {int(character.y)})",
                f"Скорость: {character.speed}",
                f"Размер кадра: {character.frame_width}x{character.frame_height}",
                "",
                "Управление:",
                "Стрелки - движение",
                "H - информация",
